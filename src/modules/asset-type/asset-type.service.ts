...
export class AssetTypeService {
  ...
  async createAssetType(createAssetTypeBody: CreateAssetTypeDto) {
    ...
    if (createAssetTypeBody.name.vi)
      await this.checkDuplicateName({
        lang: LANGUAGE.VN,
        name: createAssetTypeBody.name.vi,
      });
    ...
    const asseetType = await this.dataService.assetTypes.aggregate([
      ...
      { $unwind: '$specifications' },
      {
        $project: {
          ...
          'specifications.label.vi': { $toUpper: '$specifications.label.vi' },
        },
      },
      {
        $match: query,
  ...
  async addSpecifications(
    ...
    if (newSpecifications.specifications[0].label.vi)
      await this.checkDuplicateSpecification(params, {
        lang: LANGUAGE.VN,
        label: newSpecifications.specifications[0].label.vi,
      });
    const assetType = await this.dataService.assetTypes.findOne({
      assetTypeId: params.id,
    });
  ...
  async editSpecification(
    ...
    if (
      editedSpecification.newSpecification.label?.vi &&
      toEditSpecification.label?.vi?.toUpperCase() !==
        editedSpecification.newSpecification.label?.vi?.toUpperCase()
    ) {
      await this.checkDuplicateSpecification(params, {
        lang: LANGUAGE.VN,
        label: editedSpecification.newSpecification.label.vi,
      });
    }
    await this.dataService.assetTypes.updateOne(
      {
