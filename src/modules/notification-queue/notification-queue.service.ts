...
export class NotificationQueueService {
    ...
    if (filter.hasOwnProperty('textSearch')) {
      ...
      Object.assign(match, {
        ...
        $or: [
          ...
          { 'title.en': Utils.queryInsensitive(textSearch) },
          { 'title.ja': Utils.queryInsensitive(textSearch) },
          { 'title.cn': Utils.queryInsensitive(textSearch) },
          { 'title.vi': Utils.queryInsensitive(textSearch) },
        ],
      });
    }
  ...
  async getDetail(notiQueueId: string) {
    ...
    const deactivatedByAdmin = admins.find(
      (admin) => admin.adminId === notiQueue.deactivatedBy,
    );
    const data = {
      createdByAdmin: createdAdmin?.fullname,
      updatedByAdmin: updateByAdmin?.fullname,
      scheduledByAdmin: scheduledByAdmin?.fullname,
      deactivatedByAdmin: deactivatedByAdmin?.fullname,
    };
