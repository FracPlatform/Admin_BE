...
export class RedemptionRequestController {
  ...
  async confirmRequest(@GetUser() user, @Param('requestId') requestId: string) {
    const data = await this.redemptionService.confirmRequest(user, requestId);
    return new ApiSuccessResponse().success(data, '');
  }
  ...
  async update(
    ...
    const response = await this.redemptionService.update(
      id,
      user,
      updateCommentDto,
    );
